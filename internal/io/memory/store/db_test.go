...
func TestTable(t *testing.T) {
	...
	tb.add(api.NewDefaultSourceTuple(map[string]interface{}{"a": 1, "b": "0"}, nil, time.Now()))
	tb.add(api.NewDefaultSourceTuple(map[string]interface{}{"a": 2, "b": "0"}, nil, time.Now()))
	tb.add(api.NewDefaultSourceTuple(map[string]interface{}{"a": 3, "b": "4"}, nil, time.Now()))
	tb.add(api.NewDefaultSourceTuple(map[string]interface{}{"a": 1, "b": "1"}, nil, time.Now()))
	...
	tb.add(api.NewDefaultSourceTuple(map[string]interface{}{"a": 5, "b": "0"}, nil, time.Now()))
	tb.delete(3)
	tb.add(api.NewDefaultSourceTuple(map[string]interface{}{"a": 1, "b": "1"}, nil, time.Now()))
	v, _ = tb.Read([]string{"b"}, []interface{}{"0"})
	exp = []api.SourceTuple{
		api.NewDefaultSourceTuple(map[string]interface{}{"a": 2, "b": "0"}, nil, time.Now()),
		api.NewDefaultSourceTuple(map[string]interface{}{"a": 5, "b": "0"}, nil, time.Now()),
	}
	if len(v) != 2 {
