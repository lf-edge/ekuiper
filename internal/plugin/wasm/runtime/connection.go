...
func (r *NanomsgReqChannel) SendCmd(arg []byte) error {
	fmt.Println("[internal][plugin][wasm][runtime][connect.go] (r *NanomsgReqChannel)SendCmd: start")
	r.Lock()
	defer r.Unlock()
...
func CreateControlChannel(pluginName string) (ControlChannel, error) {
	fmt.Println("[plugin][wasm][runtime/connection.go][CreateControlChannel] start :")
	var (
		sock mangos.Socket
		err  error
	)
