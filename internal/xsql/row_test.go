...
func TestCollectionRow(t *testing.T) {
	tests := []struct {
		...
		{
			rowC:     &Tuple{Emitter: "a", Message: map[string]interface{}{"a": 1, "b": "2"}, Timestamp: timex.GetNow(), Metadata: nil},
			value:    []string{"a", "b"},
			wildcard: []string{""},
...
func TestRow(t *testing.T) {
	...
	tests := []struct {
		...
		{
			rowO: &Tuple{Emitter: "a", Message: map[string]interface{}{"a": 1, "b": "2"}, Timestamp: timex.GetNow(), Metadata: nil},
			set: [][]map[string]interface{}{
				{
					{"a": 2, "c": "3", "@d": 4},
					{"a": 3},
