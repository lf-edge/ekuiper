...
func TestGetSourceMeta(t *testing.T) {
	...
	gSourceproperty[gFile] = source

	showMeta, err := GetSourceMeta(gPlugin, "zh_CN")
	if nil != err {
		t.Error(err)
	}
	...
	if err := AddSourceConfKey(gPlugin, "new", "zh_CN", []byte(addData)); nil != err {
		t.Error(err)
	}
	...
	if err := DelSourceConfKeyField(gPlugin, "new", "zh_CN", []byte(delData)); nil != err {
		t.Error(err)
	}
