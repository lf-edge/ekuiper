...
func (suite *ServerTestSuite) SetupTest() {
	suite.s = new(Server)
	sql := "DROP STREAM plugin;"
	var reply string
	_ = suite.s.Stream(sql, &reply)
	_ = suite.s.DropRule("rule1", &reply)
}

func (suite *ServerTestSuite) TestStream() {
	sql := `Create Stream demo () WITH (DATASOURCE="../internal/server/rpctest/data/test.json", FORMAT="JSON", type="file");`
	var reply string
	err := suite.s.Stream(sql, &reply)
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), "Stream demo is created.\n", reply)

	reply = ""
	sql = "show streams;"
	err = suite.s.Stream(sql, &reply)
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), "demo\n", reply)
	...
	for i := 0; i < 5; i++ {
		...
		err = suite.s.GetQueryResult("demo", &queryresult)
		assert.Nil(suite.T(), err)
		result += queryresult
	}
...
func (suite *ServerTestSuite) TestRule() {
	...
	rule := `{
			  ...
			  "actions": [{
				"file": {
				  "path": "../internal/server/rpctest/data/result.txt",
				  "interval": 5000,
				  "fileType": "lines",
				  "format": "json"
				}
	...
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), "{\"status\": \"stopped\", \"message\": \"Stopped: canceled manually.\"}", reply)

	reply = ""
	err = suite.s.ShowRules(1, &reply)
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), "[\n  {\n    \"id\": \"myRule\",\n    \"name\": \"myRule\",\n    \"status\": \"Stopped: canceled manually.\"\n  }\n]", reply)
...
func (suite *ServerTestSuite) TestImportAndExport() {
	...
	reply = ""
	file = "rpctest/data/export.json"
	err = suite.s.Export(file, &reply)
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), "exported 1 streams, 0 tables and 1 rules", reply)
	os.Remove(file)
}

func (suite *ServerTestSuite) TestConfigurarion() {
	importArg := model.ImportDataDesc{
		FileName: "rpctest/data/import_configuration.json",
		Stop:     false,
		Partial:  false,
	}
	var reply string
	...
	exportArg := model.ExportDataDesc{
		FileName: "rpctest/data/export_configuration.json",
		Rules:    []string{},
	}
	err = suite.s.ExportConfiguration(&exportArg, &reply)
	assert.Nil(suite.T(), err)
	assert.Equal(suite.T(), "export configuration success", reply)
	os.Remove("rpctest/data/export_configuration.json")
}

func (suite *ServerTestSuite) TearDownTest() {
	// Clean up
	sql := "DROP STREAM demo;"
	var reply string
	_ = suite.s.Stream(sql, &reply)
