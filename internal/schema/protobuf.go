...
func (p *PbType) Infer(_ api.Logger, schemaId string) (ast.StreamFields, error) {
	fileId := ""
	messageId := ""
	if schemaId != "" {
	...
	ffs, err := GetSchemaFile(modules.PROTOBUF, fileId)
	if err != nil {
		return nil, err
	}
	if fds, err := protoParser.ParseFiles(ffs.SchemaFile); err != nil {
		...
		if messageDescriptor == nil {
			return nil, fmt.Errorf("message type %s not found in schema file %s", messageId, fileId)
		}
