...
func splitSink(tp *topo.Topo, s api.Sink, sinkName string, options *def.RuleOption, sc *node.SinkConf, templates []string, schema map[string]*ast.JsonStreamField) ([]node.TopNode, error) {
	...
	if batchEnabled {
		if isBytesCollector {
			...
			// from per-row in transform to once per batch here. This is a
			// single handoff: a nil return keeps per-row evaluation as is.
			if len(templates) > 0 {
				batchWriterOp.SetPropsEval(transformOp.DeferPropsEval())
			}
