...
func TestTransformSourceNode(t *testing.T) {
	...
	assert.NoError(t, err)
	decodeNode, err := node.NewDecodeOp(tp.GetContext(), "2_decoder", "test", "test", &def.RuleOption{SendError: false}, &ast.Options{TYPE: "mqtt"}, false, false, schema, nil)
	assert.NoError(t, err)
	decomNode, err := node.NewDecompressOp("2_decompressor", &def.RuleOption{SendError: false}, "gzip")
	assert.NoError(t, err)
	decodeNode2, err := node.NewDecodeOp(tp.GetContext(), "3_decoder", "test", "test", &def.RuleOption{SendError: false}, &ast.Options{TYPE: "mqtt"}, false, false, schema, nil)
	assert.NoError(t, err)
