...
func TestExplainAggInWhere(t *testing.T) {
	...
	testcases := []struct {
	...
	}{
		{
			...
			explain: `{"op":"ProjectPlan_0","info":"Fields:[ * ]"}
	{"op":"AggFunc_1","info":"aggFuncs:[$$agg_ref_0:Call:{ name:avg, args:[stream.a] }]"}
			{"op":"WindowPlan_2","info":"{ length:2, windowType:COUNT_WINDOW, collectCondition: binaryExpr:{ stream.a > Call:{ name:bypass, args:[$$default.$$agg_ref_0] } }, limit: 0 }"}
					{"op":"DataSourcePlan_3","info":"StreamName: stream, StreamFields:[ a, b ]"}`,
		},
	}
	for _, tc := range testcases {
