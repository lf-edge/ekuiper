...
func (ems *EdgexMsgBusSink) getClient(ctx api.StreamContext) error {
	log := ctx.GetLogger()
	con, err := ctx.GetConnection("edgex", ems.config)
	if err != nil {
		log.Errorf("The edgex client for connection %v get fail with error: %s", ems.config, err)
		return err
	}
	ems.client = con.(messaging.MessageClient)
	log.Infof("The edge client for connection %v get successfully", ems.config)
	return nil
}
...
func (ems *EdgexMsgBusSink) Close(ctx api.StreamContext) error {
	...
	ctx.ReleaseConnection(ems.config)
