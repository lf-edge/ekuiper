						...
						case []interface{}:
							for _, v := range r {
								if vc, ok := v.(map[string]interface{}); ok {
									...
									ctx.GetLogger().Warnf("only map[string]interface{} and []map[string]interface{} is supported but got: %v", v)
									o.statManager.IncTotalExceptions(fmt.Sprintf("only map[string]interface{} and []map[string]interface{} is supported but got: %v", v))
								}
							}
						default:
							e := fmt.Errorf("run decode op error: expect *xsql.Tuple type but got %[1]T(%[1]v)", d)
							o.Broadcast(e)
