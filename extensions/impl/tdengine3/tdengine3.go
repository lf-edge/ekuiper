...
func (t *tdengineSink3) Collect(ctx api.StreamContext, item api.MessageTuple) error {
	...
	sqlStr, _ := t.cfg.buildSql(ctx, item)
	insertSqlStr += sqlStr
	ctx.GetLogger().Debugf("tdengine3 sink collect sql: %s", insertSqlStr)
	_, e := t.cli.Exec(insertSqlStr)
	if e != nil {
...
func (t *tdengineSink3) CollectList(ctx api.StreamContext, items api.MessageTupleList) error {
	...
	items.RangeOfTuples(func(_ int, tuple api.MessageTuple) bool {
		sqlStr, _ := t.cfg.buildSql(ctx, tuple)
		insertSqlStr += sqlStr
		return true
	})
	ctx.GetLogger().Debugf("tdengine3 sink CollectList sql: %s", insertSqlStr)
...
func (cfg *TaosConfig) buildSql(ctx api.StreamContext, item api.MessageTuple) (string, error) {
	mapData := item.ToMap()
