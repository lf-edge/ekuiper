...
func TestSQLReconnect(t *testing.T) {
	...
	// With the database down, the initial connect keeps waiting for the
	// pooled connection; it must return promptly once the rule context is
	// canceled.
	ctx1, cancel := rootCtx.WithCancel()
	connectErr := make(chan error, 1)
	go func() {
		connectErr <- sqlSource.Connect(ctx1, func(status string, message string) {
			// do nothing
		})
	}()
	select {
	case err := <-connectErr:
		t.Fatalf("connect must wait while the database is down, got %v", err)
	case <-time.After(500 * time.Millisecond):
	}
	cancel()
	require.Error(t, <-connectErr)
	// Model the rule stop: the failed connector still detaches its
	// reference so the anonymous pool entry is cleaned up.
	require.NoError(t, sqlSource.Close(rootCtx))

	// Once the database is back, the source connects and reads.
	s, err := testx.SetupEmbeddedMysqlServer(address, port)
	require.NoError(t, err)
	defer s.Close()
	ctx2 := mockContext.NewMockContext("1", "2")
	require.NoError(t, sqlSource.Connect(ctx2, func(status string, message string) {
		// do nothing
	}))
	sqlConnector, ok := sqlSource.(*SQLSourceConnector)
	require.True(t, ok)
	dataChan := make(chan any, 1)
	sqlConnector.queryData(ctx2, time.Now(), func(ctx api.StreamContext, data any, meta map[string]any, ts time.Time) {
		dataChan <- data
	}, func(ctx api.StreamContext, err error) {})
	...
	ctx3, cancel3 := rootCtx.WithCancel()
	pollErr := make(chan error, 1)
	go func() {
		sqlConnector.queryData(ctx3, time.Now(), func(ctx api.StreamContext, data any, meta map[string]any, ts time.Time) {
			dataChan <- data
		}, func(ctx api.StreamContext, err error) {
			pollErr <- err
		})
