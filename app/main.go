...
func main() {
	...
	w := application.NewWindow("eKuiper一个轻量级的物联网数据分析和流处理引擎")
	application.SetIcon(fyne.NewStaticResource("icon", icon))

	lblMsg = widget.NewLabel("请先在手机权限设置中\n给予本应用读取写入存储权限\n再点击【初始化kuiperd服务】")
	...
	btnQuit = widget.NewButton("关闭kuiperd服务", func() {
		os.Exit(1)
	})
	btnQuit.Hide()
	...
	btnInit = widget.NewButton("初始化kuiperd服务", initService)
	btnStart = widget.NewButton("开启kuiperd服务", startService)
	if initialized {
		btnInit.Hide()
		btnStart.Show()
	} else {
