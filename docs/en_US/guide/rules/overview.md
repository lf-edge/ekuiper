...
| disableBufferFullDiscard | bool: false          | Whether to disable discarding the oldest data when the buffer is full and apply backpressure instead. When omitted, this defaults to `true` for QoS 1 and 2. An explicit `false` is honored, but may cause data loss during congestion.                                                                                                                |
