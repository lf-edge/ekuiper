...
| disableBufferFullDiscard | bool: false          | Whether to disable discarding the oldest data when the buffer is full and apply backpressure instead. When omitted, this defaults to `true` for QoS 1 and 2. An explicit `false` is honored, but may cause data loss during congestion. For a shared stream, backpressure from a rule using `true` propagates to the shared source and all its subscribers; this prevents channel drops but does not add checkpoint support to the shared source component. |
...
`cronDatetimeRange` supports lists of struct, you can declare a set of time ranges to express multiple time ranges for
