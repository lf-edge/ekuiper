...
| disableBufferFullDiscard | bool: false          | Legacy rule-level buffer policy. It is used when the rule's non-shared streams do not configure `BUFFER_FULL_POLICY`; when omitted, it defaults to `true` for QoS 1 and 2. Stream policy takes precedence, and all streams in a rule must resolve to the same policy. A shared stream without `BUFFER_FULL_POLICY` retains legacy `dropOldest` behavior, so backpressure must be enabled on the shared stream itself. |
