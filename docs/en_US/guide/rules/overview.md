...
| disableBufferFullDiscard | bool: false          | Legacy rule-level buffer policy used when a non-shared stream does not configure `BUFFER_FULL_POLICY`. Stream policy takes precedence, and all streams in a rule must resolve to the same policy. To enable backpressure for a shared stream, configure `BUFFER_FULL_POLICY="block"` on the stream itself; the rule option remains unsupported for a shared stream without an explicit policy. QoS does not change the buffer policy. |
