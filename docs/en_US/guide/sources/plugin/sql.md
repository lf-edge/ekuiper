...
When this source is used as a lookup table, join key values are sent as bound parameters. For backward compatibility with the previous non-Unicode SQL string literal behavior, ordinary Go string values are bound as `VARCHAR` when using SQL Server. Other value types are passed to the driver unchanged. Explicit `NVARCHAR` parameter selection is not currently configurable.
