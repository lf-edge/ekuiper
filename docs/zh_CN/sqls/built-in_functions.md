...
| 函数         | 示例                    | 说明              |
|------------|-----------------------|-----------------|
| array_position | array_postion(array, value) | 返回第二个参数在列表参数中的索引下标位置，索引下标从 1 开始，若该元素不存在，则返回 0 |
| element_at | element_at(array, index) | 返回列表参数中在给定索引下的元素，索引下标从 1 开始，若该索引小于 0，则该元素从列表末向列表头进行计数 |
