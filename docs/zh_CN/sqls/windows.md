...
条件状态窗口不关注时间，只关注每条数据对窗口状态的影响。他有1个主要参数, 窗口触发条件。

```sql
SELECT * from demo group by statewindow(a > 1)
```

单一条件状态窗口最初是未触发状态，此时所有进入窗口的数据都会被直接丢弃。当进入窗口的数据满足触发条件时，则从未触发状态转变为触发状态，此时所有进入窗口的数据都会被存储。
单一条件状态窗口处于触发状态时，当进入窗口的数据满足触发条件时，则此前所有被存储在窗口的数据都会被作为一个窗口发送，并且触发下一个窗口的数据存储。
