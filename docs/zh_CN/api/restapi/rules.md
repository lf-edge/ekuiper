...
该 API 接受 JSON 内容并执行规则 upsert：规则不存在时创建，存在时更新。

对于已有的持久化规则，校验和持久化构成更新事务的提交边界。eKuiper 会先规划候选规则，但不会替换当前运行实例。如果 JSON 解析、拓扑规划或持久化失败，请求将返回错误，原有规则定义、拓扑和运行状态均保持不变。持久化成功后，新规则定义成为权威版本，eKuiper 随后切换运行实例。此后如果运行实例启动失败，本次更新仍然保持已提交状态，不会恢复旧规则定义；规则状态 API 会将失败报告为 `stopped by error`。

更新操作不允许改变 `temp` 属性。如需在临时规则和持久化规则之间转换，请删除规则后重新创建。
