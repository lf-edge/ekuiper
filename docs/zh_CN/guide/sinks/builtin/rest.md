...
| compressionAlgorithm | 是       | 使用指定的压缩算法压缩 Payload，并在收到响应时自动对服务器响应使用指定的压缩算法进行解压缩，当前可选算法有：`zlib`, `gzip`, `flate`, `zstd`，大小写敏感。请注意此选项会将指定的算法增加到请求头 `Accept-Encoding` 中，您需要确保服务器支持您所设置的算法，同时您需要确保服务器端对于响应也应该采用相同的压缩算法对响应数据进行压缩，并在响应头中设置了 `Content-Encoding`。对于 `Flate` 算法，`Accept-Encoding` 请求头的值会设置为 `deflate`。 |
