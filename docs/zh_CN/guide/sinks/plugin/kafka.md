...
| kafkaBatchSize     | 是   | Kafka 客户端向 server 发送消息的最大缓存消息条数，默认为 1 |
| kafkaBatchBytes    | 是   | Kafka 客户端向 server 发送消息的最大缓存 bytes 容量，默认为 1048576 |
| kafkaBatchTimeout     | 是  | Kafka 客户端向 server 发送消息的缓存最大等待时间，默认为 1s     |
