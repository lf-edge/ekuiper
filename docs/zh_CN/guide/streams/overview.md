...
1. CONF_KEY: 通过 conf_key 指向配置文件中的配置项。
2. EXTRA: 快捷配置属性，覆盖 conf_key 的同名配置。

### CONF_KEY 配置

源的配置文件位于：`$ekuiper/etc/sources/{$source_type}.yaml`。yaml 文件中包含多个配置，在以下示例文件中包含 `default`,
`myconf` 和 `myconf2` 等三套配置。CONF_KEY 即指向这些 key 中的一个。
...
通过 extra 配置，可以直接在流定义的 SQL 中快速配置属性，其配置格式为 JSON 字符串。在如下示例中，流使用了 `myconf` 的配置，同时通过
extra 覆盖了 `conf2` 的值。最终的配置为 `{"conf1":"value2","conf2":1}`。
...
控制逻辑都相同，详情请参考[版本控制逻辑](../rules/overview.md#版本控制逻辑)。
