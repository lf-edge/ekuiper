...
1. 已连接，指标中用 1 表示。
2. 连接中，指标中用 0 表示。仅用于首次连接。
3. 未连接，指标中用 -1 表示。
4. 恢复中，指标中用 0 表示。仅用于曾经连接成功的连接在运行时重建传输层（自恢复客户端，或 SQL 这类由连接池恢复的客户端在恢复 worker 重试期间）；此时连接仍不可用。

上报的状态是连接池维护的最后已知状态，由驱动状态回调和周期性健康检查异步更新。读取状态（通过 API 或指标）本身不会触发网络探测。

SQL 在运行时操作失败时，第一次失败会返回给调用方并作为可疑上报给连接池；后续操作会等待，直到连接池恢复 worker 重建连接。worker 会按退避节奏一直重试直到成功，因此数据库中断只会延迟规则，不会打断规则。对从未就绪的连接做 lookup 时，首次使用会直接报不可用一次，而不是等待。

运行时重连由连接池统一负责：第一次失败上报后，后续操作会等待，连接池按退避节奏一直重试直到连接恢复。MQTT 这类自恢复客户端保留自身的自动重连，并通过同样的状态上报进度。每条规则持有共享连接上属于自己的引用；规则停止时释放引用，没有引用剩余的匿名连接会被删除。
