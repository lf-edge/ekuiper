...
内的规则会相互影响。具体表现为：源自某个规则的背压（Backpressure）可能会向后传播，影响到共享源组件。共享流上的背压进而会影响到所有连接至该共享源的其他所有规则的性能和处理。无损订阅规则会等待通道恢复容量，而不是丢弃最旧数据；停止该规则会解除订阅并释放等待。

共享流规则中的 source 部分仍不参与 checkpoint 计算。因此，共享流的无损传递可以避免内存通道丢弃数据，但不会为共享 source 组件提供 checkpoint 或 source offset 恢复能力。
